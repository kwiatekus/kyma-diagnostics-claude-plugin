...
If the user's message names a specific module or area (e.g. `eventing`, `api-gateway`, `istio`), use it as the target module throughout. Otherwise run a full cluster diagnostic.
...
> "Your KUBECONFIG uses an interactive credential plugin (`<command>`) that requires browser-based OIDC login. This will not work inside the headless Docker container used by this diagnostic.
>
> **Option A — Install kyma CLI on your host** (recommended): if `kyma` is installed locally, re-run the diagnostic and it will use your active OIDC session directly without Docker.
>
...
>
> Then set `export KUBECONFIG=/tmp/diagnostics-kubeconfig-headless.yaml` and re-run."
...
If the user named a specific module, use `--module <name>`. Otherwise run without `--module` for all modules.
...
### Confidence
<HIGH / MEDIUM / LOW> — explain briefly why>
```
...
- **Gardener** manages node pools, infrastructure, and certificates on SAP BTP. Gardener-related issues show in cluster metadata or node events.
...
- **Node topology**: Gardener provisions worker nodes across availability zones. If all nodes in one AZ show `NotReady` or pressure conditions, it is likely an infrastructure-level AZ issue rather than an application bug.
...
- **Cordoned nodes**: A node with `unschedulable: true` silently reduces cluster capacity. Pending pods and `Insufficient CPU/memory` scheduling failures may be caused by fewer schedulable nodes than expected, not by actual resource shortage.
